...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		ok := call("Coordinator.Alloc", &args, &reply)
		if ok {
			// alloc the map task
			if reply.TaskType == 0 {
				...
				// filename := "mr-out-" + strconv.Itoa(reply.TaskNumber)
				...
				filename := fmt.Sprintf("mr-out-%d", reply.TaskNumber)
				//temp_file.Close()
